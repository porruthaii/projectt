...
{
	/**
	* Array that holds the Object1s of the class Infrule, 
	...
	function _addToInference($statement)
	{
		...
		switch ($predicateLabel)
		{
			case RDF_SCHEMA_URI.RDFS_SUBPROPERTYOF :
				...
				//subject and Object1 as the statement, that asked for an 
				//entailment, and having the Object1 of the statement, 
				//that created this rule as predicate.
				$infRule->setEntailment('<s>',$statement->getObject1(),'<o>');
			...
			case RDF_SCHEMA_URI.RDFS_SUBCLASSOF :
				...
				$infRule->setEntailment('<s>',new Resource(RDF_NAMESPACE_URI.RDF_TYPE),$statement->getObject1());
			...
			case RDF_SCHEMA_URI.RDFS_RANGE :
				$infRule=new InfRule();
				$infRule->setTrigger(null,$statement->getSubject(),null);
				$infRule->setEntailment('<o>',new Resource(RDF_NAMESPACE_URI.RDF_TYPE),$statement->getObject1());
				$this->infRules[]=$infRule;
				$this->_addInfRule($infRule,$statementPosition);
			break;
			...
			case OWL_URI.OWL_INVERSE_OF :
				$infRule=new InfRule();
				$infRule->setTrigger(null,$statement->getSubject(),null);
				$infRule->setEntailment('<o>',$statement->getObject1(),'<s>');
				$this->infRules[]=$infRule;
				$this->_addInfRule($infRule,$statementPosition);
			
				$infRule=new InfRule();
				$infRule->setTrigger(null,$statement->getObject1(),null);
			...
			case OWL_URI.OWL_SAME_AS :
				...
				$infRule->setEntailment($statement->getObject1(),'<p>','<o>');
				$this->_addInfRule($infRule,$statementPosition);
			
				$infRule=new InfRule();
				$infRule->setTrigger($statement->getObject1(),null,null);
				...
				$infRule->setEntailment('<s>','<p>',$statement->getObject1());
				$this->_addInfRule($infRule,$statementPosition);
			
				$infRule=new InfRule();
				$infRule->setTrigger(null,null,$statement->getObject1());
				...
				if (isset ($this->statementRuleIndex[$statementPosition]))
				{
					...
					foreach ($this->statementRuleIndex[$statementPosition] as $key => $value)
					{
						...
						if (is_a($this,'InfModelF'))
						{
							...
							{
								$Object1Label=$trigger['o']->getLabel();
							} else 
							{
								$Object1Label='null';
							}
							unset ($this->infRulesTriggerIndex['o'][$Object1Label][array_search($key,$this->infRulesTriggerIndex['o'][$Object1Label])]);
						} else
						...
						{	
							...
							{
								$Object1Label=$entailment['o']->getLabel();
							} else 
							{
								$Object1Label='null';
							}
							unset ($this->infRulesEntailIndex['o'][$Object1Label][array_search($key,$this->infRulesEntailIndex['o'][$Object1Label])]);
						}	
	...
	* Finds all triples where the label of the Object1 node matches the regular 
	* expression.
	...
	*/
	function _findRuleTriggerInIndex($statement)
	{
		...
		//a statement's Object1 matches all triggers with null and the same URI	
		$Object1Label=$statement->getLabelObject1();
		$inIndexO=array();
		if (isset($this->infRulesTriggerIndex['o']['null'])) 
			$inIndexO=array_values($this->infRulesTriggerIndex['o']['null']);
		if (isset($this->infRulesTriggerIndex['o'][$Object1Label])) 
			$inIndexO= array_merge($inIndexO,array_values($this->infRulesTriggerIndex['o'][$Object1Label]));

		//if an infrule position occurs in subject, predicate, and Object1 index, add to result array.		
	...
	* @access	private
	*/
	function _findRuleEntailmentInIndex($subject,$predicate,$Object1)
	{
		$return=array();
		...
		if(is_a($Object1,'Node'))
		{
			$Object1Label=$Object1->getLabel();
			$inIndexO=array();
			if (isset($this->infRulesEntailIndex['o']['null'])) $inIndexO=array_values($this->infRulesEntailIndex['o']['null']);
			if (isset($this->infRulesEntailIndex['o'][$Object1Label])) $inIndexO= array_merge($inIndexO,array_values($this->infRulesEntailIndex['o'][$Object1Label]));
		} else 
		{
