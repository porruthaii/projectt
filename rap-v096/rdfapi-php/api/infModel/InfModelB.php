...
class InfModelB extends InfModel 
{
	...
	*/
    function find($subject,$predicate,$Object1) 
    {
    	...
    	foreach ($this->_infFind($subject,$predicate,$Object1,array())as $statement)
    	{
	...
	* @return	Object1 array Statements
	* @access	private
	*/
    function _infFind ($subject,$predicate,$Object1, $searchStringIndex, $findOnlyFirstMatching = false, $offset = 0,$resultCount = 0 )
    {
    	$return=array();
    	//Find all matching statements in the base statements
    	$findResult=parent::find($subject,$predicate,$Object1);
    	...
    	if ($predicate == null || 
    		...
    		//Check only Rules, that the EntailmentIndex returned.
       		foreach ($this->_findRuleEntailmentInIndex($subject,$predicate,$Object1) as $ruleKey)
    		{
	    		$infRule=$this->infRules[$ruleKey];
				$serializedRuleStatement=$ruleKey.serialize($subject).serialize($predicate).serialize($Object1);
				...
				{	
	    			...
	    			//this query
		    		if ($infRule->checkEntailment($subject,$predicate,$Object1))
		    		{
		    			//Get a modified find-query, that matches statements, 
		    			//that trigger this rule
		    			$modefiedFind=$infRule->getModifiedFind($subject,$predicate,$Object1);
			    			...
			    			{	
			    				...
			    				if (!in_array($newStatement->getLabelPredicate(),$this->supportedInference))
				    				...
				    				if($this->_nodeEqualsFind($subject,$newStatement->getSubject()) && 
				    					...
				    					$this->_nodeEqualsFind($Object1,$newStatement->getObject1() ) )
				    				{
				    					//Add to results
			    						$return[]=$newStatement;
			    						$resultCount++;
	...
	*/
	...
	if(!is_a($statement,'Statement'))
		trigger_error(RDFAPI_ERROR . '(class: InfModelB; method: contains): 
			$statement has to be Object1 of class Statement', E_USER_ERROR);	
   ...
   function findFirstMatchingStatement($subject, $predicate, $Object1, $offset = 0) 
	{
		...
		//first match.	
	   	$res= $this->_infFind($subject,$predicate,$Object1,array(),true,$offset);
