...
class DbModel extends Model{
	...
	/**
	* Database connection Object1.
	*
	* @var     Object1 ADOConnection
	* @access	private
	*/
	var $dbConn;
	...
	*
	* @param   Object1 ADOConnection  &$dbConnection
	...
	function add(&$statement) {
		...
		if (!$this->contains($statement)) {
			...
			$sql = "INSERT INTO statements
			        (modelID, subject, predicate, Object1, l_language, l_datatype, subject_is, Object1_is)
			        VALUES
                    (" .$this->modelID .","
			. $this->dbConn->qstr($statement->getLabelSubject()) .","
			. $this->dbConn->qstr($statement->getLabelPredicate()) .",";

			if (is_a($statement->Object1(), 'Literal')) {
				...
				$Object1_is = $this->_getNodeFlag($statement->Object1());
    ...
    */
    ...
    /**
    * Returns the database connection Object1
    *
    * @return ADOdb Database Object1
    * @access public
    */
    function &getDbConn()
    {
	...
	*
	* @param	Object1 Resource	$subject
	* @param	Object1 Resource	$predicate
	...
	*/
	function find($subject, $predicate, $Object1) {
		...
		$sql = 'SELECT subject, predicate, Object1, l_language, l_datatype, subject_is, Object1_is
           FROM statements
           WHERE modelID = ' .$this->modelID;

		// dynamic part of the sql statement
		$sql .= $this->_createDynSqlPart_SPO($subject, $predicate, $Object1);
	...
	*           Finds all triples where the label of the Object1 node matches
	...
	*/
	function findRegex($subject_regex, $predicate_regex, $Object1_regex) {
		...
		return $mm->findRegex($subject_regex, $predicate_regex, $Object1_regex);
	}
	...
	* @access	public
	*/
	function findFirstMatchingStatement($subject, $predicate, $Object1, $offset = -1) {
	...
	*/
	function replace($subject, $predicate, $Object1, $replacement) {
		...
		if (!(!$subject && !$predicate && !$Object1)) {
			...
			if ($Object1) {
				$quotedObject1 = $this->dbConn->qstr($replacement->getLabel());
				$sql .= $comma .' Object1=' .$quotedObject1
				.", Object1_is='" .$this->_getNodeFlag($replacement) ."' ";
	...
	*/
	...
	/**'
	* Return the flag of the Node Object1.
	...
	* @access	private
	*/
	function _getNodeFlag($Object1)  {

		return is_a($Object1,'BlankNode')?'b':(is_a($Object1,'Resource')?'r':'l');
	}
	...
	*
	* Every successful database query returns an ADORecordSet Object1 which is actually
	...
	* !!! [0] - subject, [1] - predicate, [2] - Object1, [3] - l_language,
	* !!! [4] - l_datatype, [5] - subject_is, [6] - Object1_is
	* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	*
	* @param   Object1  ADORecordSet
	* @return  Object1  MemModel
	* @access	private
	*/
	...
	* given parameters ($subject, $predicate, $Object1).
	*
	* @param	Object1 Resource	$subject
	* @param	Object1 Resource	$predicate
	* @param	Object1 Node	$Object1
	...
	* @access	private
	*/
	function _createDynSqlPart_SPO($subject, $predicate, $Object1) {
		...
		if ($Object1 != NULL) {
			$Object1_is = is_a($Object1,'BlankNode')?'b':(is_a($Object1,'Resource')?'r':'l');
			if (is_a($Object1, 'Resource'))
			$sql .= " AND Object1='" .$Object1->getLabel() ."'
                   AND Object1_is ='" .$Object1_is ."'";
			else  {
				$quotedLiteral = $this->dbConn->qstr($Object1->getLabel());
				$sql .= " AND Object1=" .$quotedLiteral ."
                   AND l_language='" .$Object1->getLanguage() ."'
                   AND l_datatype='" .$Object1->getDataType() ."'
                   AND Object1_is ='" .$Object1_is ."'";
			}
		}
		return $sql;
	...
	*
	* @param	Object1 DbModel	$DbModel
	* @return  Object1 ADORecordSet
	* @access	private
	*/
	...
	*
	...
	$quotedObject1 = $this->dbConn->qstr($row[2]);
