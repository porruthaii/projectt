...
class MemModel extends Model {
	...
	*
	* -1 : no index
	*  0 : default indices over subject, predicate, Object1 separate
	*  1 : index over subject+predicate+Object1
	*  2 : index over subject+predicate
	*  3 : index over subject+Object1
	*
	* @var		int
	* @access	private
	*/
	...
	function remove($statement) {
		...
		if($this->indexed==-1){
			...
			foreach($this->triples as $key => $value) {
				if ($this->matchStatement($value, $statement->subject(), $statement->predicate(), $statement->Object1())) {
	...
	* The defaultindex is indices over subject, predicate, Object1 seperate.
	*
	* mode = 0    : indices over subject,predicate,Object1 (default)
	* mode = 1	 : index over subject+predicate+Object1
	* mode = 2 	 : index over subject+predicate
	* mode = 3	 : index over subject+Object1
	*
	* @param     int $mode
	* @access	public
	*/
	...
	* Finds all triples with $node as Object1.
	...
	*/
	...
	function find($subject,$predicate,$Object1) {
		...
		if($subject == NULL && $predicate == NULL && $Object1 == NULL)
		return $this;

		switch($this->indexed){
			case 1:
			if($subject!=NULL && $predicate != NULL && $Object1 != NULL){
				$pos=$subject->getLabel().$predicate->getLabel().$Object1->getLabel();
				return $this->_findInIndex($pos,$subject,$predicate,$Object1,1);
			}else{
				break;
			}
			...
			if($subject!=NULL && $predicate != NULL){
				...
				return $this->_findInIndex($pos,$subject,$predicate,$Object1,2);
			}else{
				break;
			}

			case 3:
			if($subject!=NULL && $Object1 != NULL){
				$pos=$subject->getLabel().$Object1->getLabel();
				return $this->_findInIndex($pos,$subject,$predicate,$Object1,3);
			}else{
				break;
			}
			case 0:
			if($subject!= null){
				...
				return $this->_findInIndex($pos,$subject,$predicate,$Object1,4);
			}
			if($predicate!= null){
				$pos=$predicate->getLabel();
				return $this->_findInIndex($pos,$subject,$predicate,$Object1,5);
			}
			if($Object1!= null){
				$pos=$Object1->getLabel();
				return $this->_findInIndex($pos,$subject,$predicate,$Object1,6);
			}
		}
		// if no index: linear search
		foreach($this->triples as $value) {
			if ($this->matchStatement($value, $subject, $predicate, $Object1))
			$res->add($value);
		}
		return $res;
	...
	* Finds all triples where the label of the Object1 node matches the regular expression.
	* Returns an empty MemModel if nothing is found.
	*
	* @param	string	$subject_regex
	* @param	string	$predicate_regex
	* @param	string	$Object1_regex
	* @return	Object1 MemModel
	* @access	public
	*/
	function findRegex($subject_regex, $predicate_regex, $Object1_regex) {
		...
		if($subject_regex == NULL && $predicate_regex == NULL && $Object1_regex == NULL)
		return $this;
		...
		foreach($this->triples as $value) {
			...
			($Object1_regex == NULL || preg_match($Object1_regex, $value->obj->getLabel()))
			) $res->add($value);
		}
	...
	* Returns the first statement of the MemModel where the Object1 equals $node.
	...
	* @return	Object1 Statement
	* @access	public
	*/
	function findFirstMatchingStatement($subject, $predicate, $Object1, $offset = 0) {

		$currentOffset = 0;
		for($i=0;$i<=$offset;$i++)
		{
			$res = $this->findFirstMatchOff($subject, $predicate, $Object1, $currentOffset);
			$currentOffset=$res+1;
		}
		if ($res != -1) {
	...
	* Returns the position of the first statement of the MemModel where the Object1 equals $node from the given
	* offset.
	...
	* @access	private
	*/
	function findFirstMatchOff($subject,$predicate, $Object1,$off) {
		...
		switch($ind){
			...
			if($subject!=NULL && $predicate != NULL && $Object1 != NULL){
				$pos=$subject->getLabel().$predicate->getLabel().$Object1->getLabel();
				return $this->_findMatchIndex($pos,$subject,$predicate,$Object1,1,$off);
			}else{
				break;
			}
			...
			if($subject!=NULL && $predicate != NULL){
				$pos=$subject->getLabel().$predicate->getLabel();
				return $this->_findMatchIndex($pos,$subject,$predicate,$Object1,2,$off);
			}else{
				break;
			}
			...
			if($subject!=NULL && $Object1 != NULL){
				$pos=$subject->getLabel().$Object1->getLabel();
				return $this->_findMatchIndex($pos,$subject,$predicate,$Object1,3,$off);
			}else{
				break;
			}
			case 0:
			if($subject!= null){
				$pos=$subject->getLabel();
				return $this->_findMatchIndex($pos,$subject,$predicate,$Object1,4,$off);
			}
			if($predicate!= null){
				$pos=$predicate->getLabel();
				return $this->_findMatchIndex($pos,$subject,$predicate,$Object1,5,$off);
			}
			if($Object1!= null){
				$pos=$Object1->getLabel();
				return $this->_findMatchIndex($pos,$subject,$predicate,$Object1,6,$off);
			}
			break;
		}
		// if no index: linear search
	...
	* @access	public
	*/
	function findCount($subject, $predicate, $Object1) {

		$res = $this->find($subject, $predicate, $Object1);
		return $res->size();
	...
	* The values of the query variables can either be RAP's Object1s (instances of Node)
	* if $returnNodes set to TRUE, or their string serialization.
	*
	* @access	public
	* @param string $queryString
	* @param boolean $returnNodes
	* @return  array   [][?VARNAME] = Object1 Node  (if $returnNodes = TRUE)
	...
	* @param string $queryString
	* @param boolean $returnNodes
	* @return  Object1 RdqlResultIterator = with values as Object1 Node  (if $returnNodes = TRUE)
	*      OR  Object1 RdqlResultIterator = with values as strings if (if $returnNodes = FALSE)
	*
	*/
	...
	* Replaces all $node Object1s beeing subject or Object1 in
	* any triple of the MemModel with the $needle node.
	*
	* @param	Object1 Node	$subject
	* @param	Object1 Node	$predicate
	* @param	Object1 Node	$Object1
	* @param	Object1 Node	$replacement
	* @access	public
	* @throws	PhpError
	*/
	function replace($subject, $predicate, $Object1, $replacement) {
		...
		foreach($this->triples as $key => $value) {
			...
			if ($this->triples[$key]->obj->equals($Object1))
	...
	* @access	private
	*/
	function matchStatement($statement, $subject, $predicate, $Object1)  {
		...
		if($Object1 != NULL && !($statement->obj->equals($Object1)))
		return false;
	...
	* @access	public
	* @return	Object1	StatementIterator
	*/
	...
	*
	* @param    String            $pos
	* @param    Object1 Subject    &$subject
	* @param    Object1 Predicate  &$predicate
	* @param    Object1 Object1	 &$Object1
	* @param    int				 &ind
	* @return   MemModel          $res
	* @access   private
	*/
	function _findInIndex($pos,&$subject,&$predicate,&$Object1,$ind){
		$res = new MemModel($this->getBaseURI());
		$res->indexed=-1;
		if (!isset($this->indexArr[$ind][$pos]))
		return $res;
		foreach($this->indexArr[$ind][$pos] as $key =>$value){
			$t=$this->triples[$value];
			if ($this->matchStatement($t,$subject,$predicate,$Object1))
			$res->add($t);
		}
		return $res;
	}
