       ...
       if ($val_1['value'] && $val_1['value']{0}=='?') {
         ...
         switch ($key) {
           case 'subject':
                    ...
                    elseif ($pattern['subject']['value'] == $pattern['Object1']['value'])
                       $sql_where .= $sql_tmp .'s' .($n+1) .'.Object1 AND';
                    break;
           case 'predicate':
                    if ($pattern['predicate']['value'] == $pattern['Object1']['value'])
                       $sql_where .= $sql_tmp .'s' .($n+1) .'.Object1 AND';
         }
         ...
         for ($i=$n+1; $i<$count_patterns; $i++)
          ...
          if ($key == 'Object1' && isset($val_1['is_literal'])) {
              $sql_where .= ' s' .($n+1) .".Object1_is='l' AND";
 ...
 * are returned as an ADORecordSet Object1, which is then passed to this function.
 *
 * @param   Object1 ADORecordSet &$recordSet
...
/**
 * Convert variable values of $queryResult to Object1s (Node).
 *
 * @param   array  &$queryResult [][?VARNAME]['value']   = string
 *                                           ['nType']   = string
 *                                           ['l_lang']  = string
 ...
 * Look first if the given variable is defined as a pattern Object1.
...
function _generateSql_SelectVar ($varName, &$index) {
  ...
  foreach ($this->parsedQuery['patterns'] as $n => $pattern)
    if ($varName == $pattern['Object1']['value']) {

       // select the Object1 label
       $sql_select .= " s" .++$n .".Object1 as _" .ltrim($varName, "?") ." , ";
       $this->rsIndexes[$varName]['value'] = $index++;
       // select the node type
       $sql_select .= " s" .$n .".Object1_is , ";
       $this->rsIndexes[$varName]['nType'] = $index++;
       // select the Object1 language
       $sql_select .= " s" .$n .".l_language , ";
       $this->rsIndexes[$varName]['l_lang'] = $index++;
       // select the Object1 dtype
