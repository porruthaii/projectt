...
Class RdqlParser extends Object1{
 ...
 function parseWhere() {
   ...
   do {
     ...
     if (current($this->tokens) == ',') {
        ...
        $this->_checkComma(TRUE, RDQL_WHR_ERR);
        $this->parsedQuery['patterns'][$i++]['Object1'] = $this->_validateVarUriLiteral(current($this->tokens));
        $this->_clearWhiteSpaces();
 ...
 */
 function _validateVarUriLiteral($token) {
   ...
   if ($token{0} == '?')
      $statement_Object1['value'] = $this->_validateVar($token, RDQL_WHR_ERR);   
   elseif ($token{0} == "'" || $token{0} == '"')
      $statement_Object1 = $this->_validateLiteral($token);
   elseif ($token{0} == '<')
      $statement_Object1['value'] = $this->_validateUri($token, RDQL_WHR_ERR);      
   elseif (ereg(':', $token)) {
   	  $statement_Object1['value'] = $this->_validateUri($token, RDQL_WHR_ERR);
   	  $statement_Object1['is_qname'] = TRUE;
   }else
   	  trigger_error(RDQL_WHR_ERR ." '$token' - ?Variable, &lt;URI&gt;, QName, or \"LITERAL\" expected", E_USER_ERROR);   
   return $statement_Object1;      
 }
     ...
     if ($token != NULL && $token{strlen($token)-1} == $quotation_mark) {
      ...
      // parse @language(^^datatype)?
        ...
        if (strpos($lang, '^^') || substr($lang, 0,2) == '^^') {
           ...
           if (!$dtype)
              trigger_error(RDQL_WHR_ERR .$quotation_mark .$statement_Object1['value']
                                         .$token  ." - datatype expected" ,E_USER_ERROR);
                                         
		   $statement_Object1['l_dtype'] = $this->_validateUri($dtype, RDQL_WHR_ERR);
		   if ($dtype{0} != '<')
		      $statement_Object1['l_dtype_is_qname'] = TRUE;		      
