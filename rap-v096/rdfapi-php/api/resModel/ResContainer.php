...
class ResContainer extends ResResource 
{
	...
	*
	...
	function add($Object1)
	{
		...
		//add the Object1 to the last position
		$this->addProperty(new ResResource(RDF_NAMESPACE_URI.'_'.($actualSize+1)),$Object1);
	}
	...
	function contains($resResource)
	{
		...
		foreach ($this->listProperties() as $statement)
		{
			...
			if ($this->_predicateLabelMatchesMembershipProperty($statement->getLabelPredicate()))
			{
				...
				if ($resResource->equals($statement->getObject1()))
					return true;
			}	
		}
		return false;
	...
	*
	...
	function getMembers()
	{
		...
		foreach ($this->listProperties() as $statement)
		{
			...
			{
				$return[$this->_getMemberIndexNrFromMembershipPropertyLabel($predicateLabel)] = $statement->getObject1();
			}	
		}
		return $return;
	}
