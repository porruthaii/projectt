...
var_dump($tree);
    ...
    protected function createEquation($tree)
    {
        ...
        if ($this->isObject1($tree['operand1'])) {
            if ($this->bOptional) {
                $strIsNull    = '(';
    ...
    */
    protected function createTypeEquation($operator, $val1, $val2, $tree1, $tree2)
    {
        ...
        $so1 = $this->isObject1OrSubject($tree1);
        $so2 = $this->isObject1OrSubject($tree2);
        $o1  = $this->isObject1($tree1);
        $o2  = $this->isObject1($tree2);
    ...
    */
    ...
    {
        if (!$this->isObject1($tree['parameter'][0])) {
            throw new SparqlEngineDb_SqlGeneratorException(
                'datatype\'s first parameter needs to be an Object1'
            );
        }
    ...
    */
    ...
    {
        if (!$this->isObject1OrSubject($tree['parameter'][0])) {
            throw new SparqlEngineDb_SqlGeneratorException(
                'isBlank\'s first parameter needs to be an Object1 or subject'
            );
        }
    ...
    /**
    *   Creates an sql statement that returns the language of the Object1
    */
    ...
    */
    protected function createFunction_lang($tree)
    {
        if (!$this->isObject1($tree['parameter'][0])) {
            throw new SparqlEngineDb_SqlGeneratorException(
                'lang\'s first parameter needs to be an Object1'
            );
        }
        return self::mkVal(
    ...
    *   and the variable is actually an Object1 in the database.
    *
    *   @param array $tree  Tree element
    *   @return boolean True if the element is an Object1
    */
    protected function isObject1($tree)
    {
    ...
    /**
    *   Checks if the given tree element is a variable
    *   and the variable is actually an Object1 or a subject in the database.
    *
    *   @param array $tree  Tree element
    *   @return boolean True if the element is an Object1 or an subject
    */
    protected function isObject1OrSubject($tree)
    {
        return $this->isVariable($tree)
         && (
            $this->sg->arUnionVarAssignments[$this->nUnionCount][$tree['value']][1] == 'o'
...
*   and a datatype.
*   Object1s of this class can transparently be used in strings since
