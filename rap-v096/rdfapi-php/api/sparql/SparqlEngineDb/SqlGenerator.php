...
/**
*   Creates sql statements from a Query Object1
*
...
class SparqlEngineDb_SqlGenerator
{
    ...
    *   in result table t1, and ?mbox is the Object1 in
    *   table t2.
    *
    ...
    *   Object1s for easy access via their character
    *   names (spo).
    *
    *   @var array
    */
    ...
    */
    ...
    /**
    *   Creates an SQL query string from the given Sparql query Object1.
    *
...
//var_dump($this->query);
    ...
    *   array.
    *
    *   @param QueryTriple  $triple                 Array containing subject, predicate and Object1
    *   @param GraphPattern $graphPattern           Graph pattern Object1
    *
    ...
    */
    function getTripleSql(QueryTriple $triple, GraphPattern $graphPattern, $arResultVars)
    {
        ...
        $bWhereEqualityObject1    = false;

        $subject    = $triple->getSubject();
        $predicate  = $triple->getPredicate();
        $Object1     = $triple->getObject1();
        ...
        */
        ...
        if (SparqlVariable::isVariable($Object1)) {
            if (isset($this->arUnionVarAssignments[$this->nUnionCount][$Object1])) {
                //already selected -> add equality check
                $bWhereEqualityObject1 = true;
                $this->arUsedVarTypes[$Object1]['o'] = true;
            } else {
                if (isset($this->arVarAssignments[$Object1][0])) {
                    $strTablePrefix = $this->arVarAssignments[$Object1][0];
                }
                $this->arVarAssignments[$Object1] = array($strTablePrefix, 'o');
                $this->arUnionVarAssignments[$this->nUnionCount][$Object1] = array($strTablePrefix, 'o');
                $this->arUsedVarTypes[$Object1]['o'] = true;
                if (self::isResultVar($Object1, $arResultVars)) {
                    $arSelect[$Object1] = $this->createVariableSelectArray(
                        'o', $Object1, $strTablePrefix
                    );
                    if (isset($this->arUsedVarAssignments[$Object1])) {
                        $arRefVars[$Object1] = $strTablePrefix . '.Object1';
                    } else {
                        $this->arUsedVarAssignments[$Object1] = $strTablePrefix . '.Object1';
                    }
                }
                if (isset($this->query->varLanguages[$Object1])
                 && $this->query->varLanguages[$Object1] !== null
                ) {
                    $strWhereEquality .=
                        ' AND ' . $strTablePrefix . '.l_language = "'
                        . addslashes($this->query->varLanguages[$Object1]) . '"';
                }
                if (isset($this->query->varDatatypes[$Object1])
                 && $this->query->varDatatypes[$Object1] !== null
                ) {
                    $strWhereEquality .=
                        ' AND ' . $strTablePrefix . '.l_datatype = "'
                        . addslashes($this->query->varDatatypes[$Object1]) . '"';
                }
            }
        }
        ...
        */
        ...
        if ($bWhereEqualityObject1) {
            $strWhereEquality .= ' AND ' . self::getSqlEqualityCondition(
                            array($strTablePrefix, 'o'),
                            $this->arVarAssignments[$Object1]
                        );
        }
        ...
        if ($this->nUnionTriplePatternCount == 0) {
            ...
            if ($graphPattern->getOptional() !== null) {
                $strFrom .=  $this->getSqlCondition($subject  , $strTablePrefix, 'subject')
                           ...
                           . $this->getSqlCondition($Object1   , $strTablePrefix, 'Object1')
                           . $strWhereEquality;
            }
        }
    ...
    protected function createVariableSelectArray($chType, $varname, $strTablePrefix)
    {
                ...
                if ($func == 'datatype') {
                    if ($chType != 'o') {
                        throw new SparqlEngineDb_SqlGeneratorException(
                            'datatype() works on Object1s only'
                        );
                    }
                    return array(
        ...
        switch ($chType) {
            ...
            case 'o':
                return array(
                    $strTablePrefix . '.Object1 as "'     . $strTablePrefix . '.' . $this->getSqlVariableNameValue($varname) . '"',
                    $strTablePrefix . '.Object1_is as "'  . $strTablePrefix . '.' . $this->getSqlVariableNameIs($varname) . '"',
        ...
        foreach ($arSelect as $nUnionCount => $arUnionVars) {
            ...
            foreach ($arVars as $strVar) {
                ...
                if ((
                    ...
                    //it's a subject or Object1, but we don't want the type
                    $ar[] = 'NULL as '
    ...
    *   of some subject/predicate/Object1
    *
    *   @param mixed    $bject          subject|predicate|Object1
    *   @param string   $strTablePrefix Table prefix (e.g. "t0")
    *   @param string   $strType        Type of $bject ('subject'|'predicate'|'Object1')
    ...
    */
    ...
    /**
    *   Checks if the sentence part (subject, predicate or Object1) in
    *   $arNew has the same content as $arOld.
    *   Required for queries like ":x ?a ?a" where predicate and Object1
    ...
    */
    protected static function getSqlEqualityCondition($arNew, $arOld)
    {
        ...
        if ($chTypeNew == 'p' || $chTypeOld == 'p') {
            //just check value
            //FIXME: it might be I need to check for resource type in Object1 and subject
            return
            ...
            //two Object1s -> check everything
            return
                  $strTablePrefixNew . '.Object1 = '     . $strTablePrefixOld . '.Object1'
                . ' AND '
                . $strTablePrefixNew . '.Object1_is = '  . $strTablePrefixOld . '.Object1_is'
                . ' AND '
