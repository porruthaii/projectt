...
class N3Parser extends Object1 {
...
//   $Literal  = '"[^"\\\\]*(?:\\.\\[^"\\]*)*"'; # '"(?:\\"|[^"])*"'
  ...
  function parse($s) {
    ...
    foreach ( $stat as $t) {
      ...
      if (count($t)>3) {
        ...
        for ($i = 3; $i < 5; $i++){
          if ($t[$i][0]=='@')$Object1.=$t[$i];
          if (substr($t[$i],0,2)=='^^')$Object1.=$t[$i];
        };
      } else {$Object1=$t[2];};

      print '('.$t[0].', '.$t[1].', '.$Object1.")\n";
  ...
  /**
   * This parses a N3 string and calls func($subject, $predicate, $Object1) with each triple
   * @param string $s
   * @param string $func
   * @access public
   **/
...
//var_dump($newres[0]);
  ...
  /**
   * Does the real work, returns a list of subject, predicate, Object1 triples.
   * @param array $list
   * @returns array
   * @access private
   **/
