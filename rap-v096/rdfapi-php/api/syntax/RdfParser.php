...
class RdfParser extends Object1 {
...
/**
   ...
   * @access	private
*/
function _report_statement( $subject_type, $subject, $predicate, $ordinal, $Object1_type,  $Object1, $xml_lang, $bag_id, $statements, $statement_id, $datatype )
{
    $statement_id_type = RDF_SUBJECT_TYPE_URI;
    $statement_id_buffer='';
    $predicate_buffer='';
    if (!$xml_lang && $Object1_type == RDF_Object1_TYPE_LITERAL && isset($this->rdf_parser['document_xml_lang']))
    	$xml_lang = $this->rdf_parser['document_xml_lang'];

	 // call add statement
	 $this->add_statement_to_model($this->rdf_parser['user_data'],$subject_type,$subject,$predicate,$ordinal,$Object1_type,$Object1,$xml_lang, $datatype );
        ...
        if( $statement_id )
        {
			...
			if ($subject_type == RDF_SUBJECT_TYPE_BNODE)
			   $obj_type = RDF_Object1_TYPE_BNODE;
			else
			   $obj_type = RDF_Object1_TYPE_RESOURCE;
            ...
            else if( ( $ordinal = $this->_is_rdf_ordinal( $attribute_local_name ) ) != 0 )
            {
				$this->_report_statement(
					...
					'' );
                     ...
                     && ($attribute_local_name != RDF_Object1)
...
function _handle_resource_element( $namespace_uri, $local_name, $attributes, $parent )
{
    ...
    if( $parent )
    {
        if ($this->rdf_parser['top']['subject_type'] == RDF_SUBJECT_TYPE_BNODE)
        	$objtype = RDF_Object1_TYPE_BNODE;
        else
        	$objtype = RDF_Object1_TYPE_RESOURCE;
...
function _handle_property_element( &$namespace_uri, &$local_name, &$attributes )
{
    ...
    if( $parse_type )
	{
        ...
        if( $resource !== NULL )
        {
        	...
        	$Object1_type=RDF_Object1_TYPE_RESOURCE;
        }
        else
        {
			$subject_type = RDF_SUBJECT_TYPE_BNODE;
            $this->_generate_anonymous_uri( $buffer );
            $Object1_type=RDF_Object1_TYPE_BNODE;
        }
        $this->rdf_parser['top']['state'] = IN_PROPERTY_EMPTY_RESOURCE;
		...
		for( $i = 0; isset($attributes[$i]); $i += 2 )
    	{
        	...
        	if( $attribute_namespace_uri == '' || $attribute_namespace_uri == RDF_NAMESPACE_URI  )
	        {
        	    if( $attribute_local_name == RDF_ABOUT ||
        	        $attribute_local_name == RDF_RESOURCE)
        	    {
					$this->rdf_parser['top']['parent']['collection']['Object1_type'][] = RDF_Object1_TYPE_RESOURCE;
        	    }
        	    elseif ( $attribute_local_name == RDF_NODEID ) {
        	    	$this->rdf_parser['top']['parent']['collection']['Object1_type'][] = RDF_Object1_TYPE_BNODE;
        	    }
        	    $this->rdf_parser['top']['parent']['collection']['Object1_label'][] = $attribute_value;
    		}
    	}
	}
*/
// new
		...
		for( $i = 0; isset($attributes[$i]); $i += 2 )
    	{
        	...
        	if( $attribute_namespace_uri == '' || $attribute_namespace_uri == RDF_NAMESPACE_URI  )
	        {
        	    ...
        	    $this->rdf_parser['top']['parent']['collection']['Object1_label'][] = $id_buffer;
...
function _end_collection() {
	...
	if (isset($this->rdf_parser['top']['collection']))
	{
		...
		for ($i=0; isset($this->rdf_parser['top']['collection']['Object1_label'][$i]); $i++)
		{
			...
			$this->_report_statement(
				...
				'',
				$this->rdf_parser['top']['collection']['Object1_type'][$i],
				$this->rdf_parser['top']['collection']['Object1_label'][$i],
				'',
				'',
				'',
				'',
			...
			if (!isset($this->rdf_parser['top']['collection']['Object1_label'][$i+1]))
			{
				$obj_type_2 = RDF_Object1_TYPE_RESOURCE;
				$Object1_2 = RDF_NAMESPACE_URI.RDF_NIL;
			}
			else
			{
				$obj_type_2= RDF_Object1_TYPE_BNODE;
				$this->_generate_anonymous_uri($Object1_2);
			}
...
echo "";
    ...
    case IN_PROPERTY_PARSE_TYPE_RESOURCE:
 		$this->rdf_parser['top']['state'] = IN_PROPERTY_UNKNOWN_Object1;
   	...
   	case IN_PROPERTY_PARSE_TYPE_COLLECTION:
		...
		break;
    case IN_PROPERTY_UNKNOWN_Object1:
        /* if we're in a property with an unknown Object1 type and we encounter
           an element, the Object1 must be a resource, */
...
function _character_data_handler( $parser,$s)
{
    ...
    case IN_PROPERTY_UNKNOWN_Object1:
        ...
        if( $this->rdf_parser['top']['state'] == IN_PROPERTY_UNKNOWN_Object1 )
        {
            /* look for non-whitespace */
            for( $i = 0; (( $i < $len ) && (preg_match('#|/n|/t+#',$s{ $i }) )); $i++ );
	...
	$datatype )
{
	...
	if(UNIC_RDF){
		...
		$Object1=$this->str2unicode_nfc($Object1);
	}
	...
	// create Object1
	if (($Object1_type == RDF_Object1_TYPE_RESOURCE) || ($Object1_type == RDF_Object1_TYPE_BNODE)) {
			if ($Object1_type == RDF_Object1_TYPE_BNODE)
					$objobj = new BlankNode($Object1);
				else
					$objobj = new Resource($Object1);
	} else {

		$objobj = new Literal($Object1);
		if ($datatype != '') {
