...
*
*/
class RdfSerializer extends Object1 {
	...
	*/
	function writeDescriptions()  {
		...
		foreach($this->m_statements as $key => $statement) {
            ...
            if (($predicate->getURI() == RDF_NAMESPACE_URI.RDF_TYPE) && is_a($Object1, 'Resource') && !$this->m_groupTypeStatement) {
                $this->writeGroup();
                $this->m_groupTypeStatement = $statement;
            } 
			elseif ($this->canAbbreviateValue($Object1) &&
			$this->use_attributes &&
			$this->checkForDoubleAttributes($predicate))
			{
				if (is_a($Object1, 'Literal')) {
					if ($Object1->getDatatype() == NULL) {
	...
	*/
	...
	/**
	* @param Object1 Node $subject_node
	*
	* @access   private
	*/
	...
	*/
	...
	/**
	* @param Object1 $Object1_node
	* @access   private
	*/
	function writeResourceReference($Object1_node)  {
		$rebaseURI = $Object1_node->getURI();
		$this->m_out .= ' ';
		if (is_a($Object1_node, 'BlankNode')) {
			$this->m_out .= $this->rdf_qname_prefix . RDF_NODEID;
		} else {
	...
	function collectNamespaces($model)  {
		foreach($model->triples as $key => $value) {
			...
			if ($this->use_entities) {
				...
				if(!is_a($value->getObject1(), 'Literal'))
				$this->collectNamespace($value->getObject1());
