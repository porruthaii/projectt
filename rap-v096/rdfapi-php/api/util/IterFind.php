...
class IterFind
{
    ...
    * @param $Object1 Resource
    * @param $returnAsQuads boolean
	* @access	public
    */
	function IterFind($graph,$subject,$predicate,$Object1,$returnAsQuads=false)
	{
		if ($graph==NULL)
		{
			$this->valid=false;
    ...
    */
	function next()
	{
		$this->current = $this->findGraph->findFirstMatchingStatement($this->findSubject,$this->findPredicate,$this->findObject1,++$this->key);
    ...
    */
	function current()
	{
		...
		return new Quad(new Resource($this->findGraph->getGraphName()),$this->current->getSubject(),$this->current->getPredicate(),$this->current->getObject1());
		//else
		return $this->current;
	}
