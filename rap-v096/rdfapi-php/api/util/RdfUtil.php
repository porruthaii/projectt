...
class RDFUtil extends Object1 {
	...
	*
	* @param	Object1 Resource	$resource
	* @return	string
	* @access	public
	*/
	...
	*/
	function writeHTMLTable(&$model)  {
		...
		foreach($model->triples as $key => $statement) {
			...
			// Object1
			echo INDENTATION . INDENTATION . '<td bgcolor="';
			echo RDFUtil::chooseColor($statement->getObject1());
			echo '">';
			echo '<p>';
			if (is_a($statement->getObject1(), 'Literal')) {
			...
			echo  RDFUtil::getNodeTypeName($statement->getObject1())
 ...
 */
 ...
 function visualizeGraph(&$model, $format = "input_dot", $short_prefix = TRUE)
 {
     ...
     foreach ($model->triples as $key => $statement)
     {
         ...
         $Object1    = $statement->getLabelObject1();
	...
	if (is_a($statement,'InfStatement'))
         ...
         // format Object1
         if (!isset($attrib[$Object1]))
         {
             if (is_a($statement->Object1(),'BlankNode'))
             {
                 $attrib[$Object1] = $graphviz_param['BLANKNODE_STYLE'];
             }
             elseif (is_a($statement->Object1(),'Literal'))
             {
                 $Object1_label = $Object1;
                 $Object1 = "literal".$i;
                 $i++;
                 $attrib[$Object1] = "label=\"$Object1_label\" ".$graphviz_param['LITERAL_STYLE'];
             }
             else
             {
                 if ($short_prefix == TRUE && RDFUtil::guessPrefix($Object1, $model) != FALSE)
                 {
                     $prefix = RDFUtil::guessPrefix($Object1, $model);
                     $Object1_label = $prefix.":".RDFUtil::guessName($Object1);
                     $attrib[$Object1] = "label=\"".$Object1_label."\" ";
                     if(!isset($prefix_array[$prefix]))
                     {
                         $prefix_array[$prefix] = RDFUtil::guessNamespace($Object1);
                     }
                 }
                 if (GRAPHVIZ_URI == TRUE)
                 {
                     $attrib[$Object1] .= "URL=\"".$Object1."\"";
                 }
             }
         }

         // fill graph array
         $graph[] = "\"".$subject."\" -> \"".$Object1."\" [".$predicate_label."];";
     }
