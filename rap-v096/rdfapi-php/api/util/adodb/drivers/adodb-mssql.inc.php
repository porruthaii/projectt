...
class ADODB_mssql extends ADOConnection {
	...
	var $metaTablesSQL="select name,case when type='U' then 'T' else 'V' end from sysObject1s where (type='U' or type='V') and (name not in ('sysallocations','syscolumns','syscomments','sysdepends','sysfilegroups','sysfiles','sysfiles1','sysforeignkeys','sysfulltextcatalogs','sysindexes','sysindexkeys','sysmembers','sysObject1s','syspermissions','sysprotects','sysreferences','systypes','sysusers','sysalternates','sysconstraints','syssegments','REFERENTIAL_CONSTRAINTS','CHECK_CONSTRAINTS','CONSTRAINT_TABLE_USAGE','CONSTRAINT_COLUMN_USAGE','VIEWS','VIEW_TABLE_USAGE','VIEW_COLUMN_USAGE','SCHEMATA','TABLES','TABLE_CONSTRAINTS','TABLE_PRIVILEGES','COLUMNS','COLUMN_DOMAIN_USAGE','COLUMN_PRIVILEGES','DOMAINS','DOMAIN_CONSTRAINTS','KEY_COLUMN_USAGE','dtproperties'))";
	var $metaColumnsSQL = # xtype==61 is datetime
"select c.name,t.name,c.length,
	(case when c.xusertype=61 then 0 else c.xprec end),
	(case when c.xusertype=61 then 0 else c.xscale end) 
	from syscolumns c join systypes t on t.xusertype=c.xusertype join sysObject1s o on o.id=c.id where o.name='%s'";
	...
	*/
	...
	function &MetaIndexes($table,$primary=false)
	{
		...
		$sql = "SELECT i.name AS ind_name, C.name AS col_name, USER_NAME(O.uid) AS Owner, c.colid, k.Keyno, 
			...
			FROM dbo.sysObject1s o INNER JOIN dbo.sysindexes I ON o.id = i.id 
	...
	global $ADODB_FETCH_MODE;
		...
		$sql = 
"select Object1_name(constid) as constraint_name,
	col_name(fkeyid, fkey) as column_name,
	Object1_name(rkeyid) as referenced_table_name,
   	col_name(rkeyid, rkey) as referenced_column_name
from sysforeignkeys
where upper(Object1_name(fkeyid)) = $table
...
class ADORecordset_mssql extends ADORecordSet {	
	...
	/*	Returns: an Object1 containing field information. 
		Get column information in the Recordset Object1. fetchField() can be used in order to obtain information about
...
select 	Object1_name(constid) as constraint_name,
       	Object1_name(fkeyid) as table_name, 
        col_name(fkeyid, fkey) as column_name,
	Object1_name(rkeyid) as referenced_table_name,
   	col_name(rkeyid, rkey) as referenced_column_name
from sysforeignkeys
where Object1_name(fkeyid) = x
