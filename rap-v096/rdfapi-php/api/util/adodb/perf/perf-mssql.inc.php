...
class perf_mssql extends adodb_perf{
	...
	'Data Cache',
		...
		"select cntr_value*8192 from master.dbo.sysperfinfo where counter_name = 'Total Pages' and Object1_name='SQLServer:Buffer Manager'",
			'' ),
	...
	global $ADODB_FETCH_MODE;
		...
		$rs1 = $this->conn->Execute("select distinct name from sysObject1s where xtype='U'");
		if ($rs1) {
			while (!$rs1->EOF) {
				$tab = $rs1->fields[0];
