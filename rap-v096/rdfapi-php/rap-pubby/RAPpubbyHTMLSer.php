...
Class RAPpubbyHTMLSer extends Object1 {
	...
	*/		
	function findImgLink (&$m, &$res) {
		...
		foreach ($_PUBBY_HTML_SER['imageProperty'] as $prop) {
			...
			if ($triple) {
				return $triple->getObject1()->getURI();
			}
		}
		return '';
	}
	...
	function findLiteralValueLang(&$m, &$res, &$prop_array) {
		...
		if (PUBBY_HTML_SER__DEFAULT_LANG == '') {
			...
			foreach ($prop_array as $prop) {
				...
				$triple = $m->findFirstMatchingStatement($res, $prop, NULL);
				if ($triple) {
					return $triple->getObject1()->getLabel();
				}
			}
		}
		else {
			foreach ($prop_array as $prop) {
				...
				while ($iter->hasNext()) {	
					...
					$triple = $iter->next();				
					if ($triple->getObject1()->getLanguage() == PUBBY_HTML_SER__DEFAULT_LANG) {						
						return $triple->getObject1()->getLabel();
					}
				}
				...
				if (!$result->isEmpty()) {
					...
					if ($triple) {
						$tmp = $triple->getObject1()->getLabel();
					}
				}
			}
		}									
	...
	*/	
	function renderResourceProperties (&$m, &$res) {
		...
		while ($iter->hasNext()) {	
			...
			$prop[$propURI][false][] = $triple->getObject1();			
		}
		...
		while ($iter->hasNext()) {	
			$triple = $iter->next();
			$propURI = $triple->getPredicate()->getURI();
			// inverse the property --> get subject instead of Object1
