	...
	*/
	function testRdfsFModel()
	{
		...
		do 
		{
			...
			$inf->load(RDFS_INF_TESTFILES.$findTBOX->triples[0]->getLabelObject1());
	
			$findDATA=$res1->find(null,new Resource($testURI.'data'),null);
			$inf->load(RDFS_INF_TESTFILES.$findDATA->triples[0]->getLabelObject1());
						
			$findQUERY=$res1->find(null,new Resource($testURI.'query'),null);
			$query =$this->_doFindFromFile(RDFS_INF_TESTFILES.$findQUERY->triples[0]->getLabelObject1(),$inf);
			
			$result = new MemModel();
			$findRESULT=$res1->find(null,new Resource($testURI.'result'),null);
			$result->load(RDFS_INF_TESTFILES.$findRESULT->triples[0]->getLabelObject1());
			...
			echo '<b>'.$findDATA->triples[0]->getLabelObject1().' (RDFSFModel)</b><BR>';
	...
	function _doFindFromFile($file,& $model)
	{
		...
		foreach ($mod->triples as $statement)
		{
			...
			if (in_array($statement->getLabelObject1(),$nullVarURIs))
			{
				$findO=null;
			} else 
			{
				$findO=	$statement->getObject1();
			};
