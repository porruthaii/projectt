...
class testMm_search_test extends UnitTestCase {
	...
	*/
 	function testFindTest(){
 		...
 		for($i=-1;$i<4;$i++){
			...
			$this->assertNotEqual(null,$model1->find($needle->getSubject(),$needle->getPredicate(),$needle->getObject1())); 		
 		}
	...
	*/
 	function testFindSubTest(){
 		...
 		for($i=-1;$i<4;$i++){
 		 ...
 		 for($i=-1;$i<4;$i++){
 			$model1= $this->_generateModel(100,2,$i,$needle);
			$this->assertNotEqual(null,$model1->find(null,null,$needle->getObject1())); 		
 		}
	...
	*/
 	function testFindFirstTest(){
 		...
 		$model1=new MemModel(); 		
 		$needle=new Statement(new Resource('http://www.example.org/Subject3'),new Resource('http://www.example.org/pred'),new Resource('http://www.example.org/ob'));
 		$stat=new Statement(new Resource('http://www.example.org/Subject0'),new Resource('http://www.example.org/Predicate0'),new Resource('http://www.example.org/Object10'));
 		for($i=-1;$i<4;$i++){
 			$model1= $this->_generateModel(100,2,$i,$needle);
...
//===================================================================
	...
	* and given indextype.
    *
    * @return Object1 MemModel $model
    * @param  int $num 
    * @param  int $des
    * @param  int $ind
    * @param  Object1 MemModel $needle
    */
	...
	function _generateModel($num,$des,$ind,$needle){
		...
		for($i=0;$i<$num;$i++){
 		...
 		// generate Object1s
 		for($i=0;$i<$num;$i++){
 			$objs[$i]=new Resource('http://www.example.org/Object1'.$i%5);
 		}
 		for($i=0;$i<$num;$i++){
 						if($i==50){
 							$model->add($needle);
