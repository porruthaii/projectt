 ...
 class testResModel_Property_tests extends UnitTestCase {
	...
	*/
 	function testGetProptertyTest(){
		...
		$stat=new Statement(new Resource('http://www.example.org/needle'),new Resource('http://www.example.org/pred'),new Literal('Object1'));
		$model1->add($stat);
		...
		$res=$resmodel->getProperty($subject,$property);
 		$this->assertEqual('Triple(Resource("http://www.example.org/needle"), Resource("http://www.example.org/pred"), Literal("Object1"))',$res->toString());
 	    $model1->close();
 	}
	...
	*/
 	function testCreateProptertyCreateResourceTest(){
		...
		$resresource->addProperty($prop,new ResLiteral('Object1'));		
		$subject=new ResResource('http://www.example.org/testresource');
		$property=new ResResource('http://www.example.org/pred');		
		$res=$resmodel->getProperty($subject,$property);
 		$this->assertEqual('Triple(Resource("http://www.example.org/testresource"), Resource("http://www.example.org/pred"), Literal("Object1"))',$res->toString());
 	    $model1->close();
 	}
