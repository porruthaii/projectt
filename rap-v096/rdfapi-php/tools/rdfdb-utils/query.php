...
function doNada() {}
  ...
  function del(s,p,o) {  
    if (confirm("Are you sure you want to delete the triple ("+s+", "+p+", "+o+")?")) { 
      ...
      document.getElementById("tripleObject1").value=o;
...
function search($m, $s, $p, $o) {
  ...
  //Can't put Object1s in session, so preproc.
  $triples=array();
...
?>
   ...
   <?php   if(isset($_REQUEST["rdql"])){
	...
	if (isset($_REQUEST["literal"])) 
	  $o=$_REQUEST["Object1"]!=""?new Literal($_REQUEST["Object1"]):null;
	else 
	  $o=$_REQUEST["Object1"]!=""?new Resource($_REQUEST["Object1"]):null;
	...
	$_SESSION["qObject1"]=$o==null?null:$o->toString();
      } else { 	
	...
	if ($triples===FALSE) { 
	  ...
	  $o=nodeFromString($_SESSION["qObject1"]);
	  $triples=search($m,$s,$p,$o);
	  $_SESSION["triples"]=$triples;
      ...
      print "<input type='hidden' id='tripleObject1' name='tripleObject1' />\n";
	...
	$atriples=$triples; 
      ...
      print "<table class='resulttable'>\n<tr><td style='width: 64px'>------------</td><td class='tableheader'>Subject</td><td class='tableheader'>Predicate</td><td class='tableheader'>Object1</td></tr>\n";
    ...
    if ($op=="delete") { 
      ...
      $o=nodeFromString($_REQUEST["tripleObject1"]);
...
?>
	  ...
	  <input name="otripleObject1" type="hidden" value="<?php print $oo?>"/>
	  ...
	  <span class="tableheader">Object1:</span><br/>
	  <input name="tripleObject1" type="text" size="120" value="<?php print $o?>"/><br/>
...
if ($op=="editsave") { 
  ...
  $o=nodeFromString($_REQUEST["otripleObject1"]);
  
  print "<!--";
  var_dump($_REQUEST["otripleSubject"]);
  var_dump($_REQUEST["otriplePredicate"]);
  var_dump($_REQUEST["otripleObject1"]);
  print "-->";
  ...
  if ($_REQUEST["literal"]) 
    $o=new Literal($_REQUEST["tripleObject1"]);
  else 
    $o=new Resource($_REQUEST["tripleObject1"]);
...
<p>Leave blank to list all</p>
   ...
   <span class="tableheader">Object1:</span><br/>
   <input type="text" name="Object1" size="120" />
   <span class="tableheader">Literal:</span>
